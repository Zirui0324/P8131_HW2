...
c(x0fit, sqrt(varx0))
10^(x0fit+c(qnorm(0.05), -qnorm(0.05))*sqrt(varx0))
```
...
```{r}
glm_log = glm(resp~x, family=binomial(link='log'))
summary(glm_log)
alfa = coef(glm_log)[1]
beta = coef(glm_log)[2]
...
sum(residuals(glm_logit, type='pearson')^2)
dev = sum(residuals(glm_logit, type='deviance')^2)
dev
...
beta
se = sqrt(vcov(glm_logit)[2,2])
beta+c(qnorm(0.025), -qnorm(0.025))*se
```
```{r}
betacov = vcov(glm_logit)
x40 = log(0.4/(1-0.4))-alfa/beta
varx40 = betacov[1,1]/(beta^2)+betacov[2,2]*(alfa^2)/(beta^4)-2*betacov[1,2]*alfa/(beta^3)
c(x40, sqrt(varx40))
